...
**文档版本**: v2.1.0
**最后更新**: 2026-05-07
**项目状态**: 数字人 Live2D + 语音交互已完成集成，表情系统已修复
...
| 分类 | 说明 |
|------|------|
| AI 助手回复异常 | **根因分析**：AI 返回的 `Response` 对象文本在 `output` 属性中，原文本提取函数遗漏了对此属性的解析，导致大模型回答提取为空并触发预设的错误提示。**解决方案**：修改 `extract_answer_fragment`，增加 `output` 属性解析支持，并过滤大模型的思考过程，恢复正常的 AI 交互体验 |
